...
func %(className)Add(xlsx *excelize.File) []faultmodelstruct.%(className){
	%(name)Rows := xlsx.GetRows("%(sheetName)")
	if len(%(name)Rows) == 0 {
...
func Calculate%(sheetName)P(observation [][]float64, beliefP float64, transP float64, GaussCoefficient [][]float64,index int,faults *faultmodelstruct.Fault,common *commonStruct.Common) ([]float64, []bool) {
